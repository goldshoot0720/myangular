...
import { sub4data } from './sub4data';
import { CommonModule } from '@angular/common';
import { createClient } from '@supabase/supabase-js';
import {MatTableModule} from '@angular/material/table';
import {sub9data} from '../sub9/sub9data';
...
@Component({
  selector: 'app-sub4',
  imports: [RouterLink, FormsModule, CommonModule, MatTableModule],
...
export class Sub4Component implements OnInit {
  ...
  async onButton1Click() {
    if (!this.textareavalue1 || !this.textareavalue2 || !this.textareavalue3) {
      alert("訂閱項目不得為空值\n訂閱費用不得為空值\n訂閱到期日不得為空值");
    } else {
      // Check if the subscription already exists
      const isDuplicate = this.sub4data.some(item =>
      ...
      if (isconfirm) {
        ...
        if (error) {
          ...
          // Add the new data to the sub4data array and sort it
          this.sub4data = [...this.sub4data, insertedData[0]]
            .sort((a, b) => new Date(a.textareavalue3).getTime() - new Date(b.textareavalue3).getTime());
  ...
  async saveEdit(data: any) {
    ...
    if (error) {
      ...
      this.sub4data = this.sub4data.map(item =>
    ...
    if (isconfirm) {
      const deleteData = async () => {
        ...
        if (error) {
          ...
          this.sub4data = this.sub4data.filter(item => item.id !== data.id)
            .sort((a, b) => new Date(a.textareavalue3).getTime() - new Date(b.textareavalue3).getTime());
          alert("資料刪除成功！");
        }
      };
  ...
  calculatePercentage(value: string): number {
    ...
    let myfloat = (numerator / denominator) * 100;
    return parseFloat(myfloat.toFixed(2)); // Convert the string result of toFixed back to a number
  }

  calculateMinus(value: string): number {
    const [numerator, denominator] = value.split('/').map(Number);
    let myminus = numerator - denominator;
    return myminus; // Convert the string result of toFixed back to a number
  }
