...
@Component({
  ...
  styleUrls: ['./sub9.component.css']
})

export class Sub9Component implements OnInit {

  protected readonly mainlist = mainlist;
  protected sub9data = sub9data;
  ...
  async onButton1Click() {
    if (!this.textareavalue1 || !this.textareavalue2 || !this.textareavalue3) {
      ...
      const isDuplicate = this.sub9data.some(item =>
      ...
      if (isconfirm) {
        ...
        if (error) {
          ...
          // Add the new data to the sub9data array and sort it
          this.sub9data = [...this.sub9data, insertedData[0]]
            .sort((a, b) => new Date(a.textareavalue3).getTime() - new Date(b.textareavalue3).getTime());
  ...
  async saveEdit(data: any) {
    ...
    if (error) {
      ...
      this.sub9data = this.sub9data.map(item =>
    ...
    if (isconfirm) {
      const deleteData = async () => {
        ...
        if (error) {
          ...
          this.sub9data = this.sub9data.filter(item => item.id !== data.id)
            .sort((a, b) => new Date(a.textareavalue3).getTime() - new Date(b.textareavalue3).getTime());
          alert("資料刪除成功！");
        }
      };
